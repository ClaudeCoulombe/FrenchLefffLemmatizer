...
def loadLefffDict(INPUT_FILE_PATH):
    ...
    with open(INPUT_FILE_PATH, mode='r', encoding='utf-8') as input_file:
        for input_line in input_file:
            ...
            if (re.search(
                    '^bée\t|^dû\t|^dûs\t|^due\t|^dues\t|^dus\t|^inf\.\.\t|^messis\t|^messise\t|^messises\t|^pu\t|^sup\.\.\t',
                    input_line, flags=0) != None):
                pass
            ...
            #           elif ( re.search("([^\d\s]*)\tadj\t([^\d\s]*[r]+)\t([^\d\s]*)|([^\d\s]*)\tadj\t([^\d\s]*re+)\t([^\d\s]*)",input_line, flags=0) != None ):
                ...
                if (clitic_pronoun != None):
                    pos_tag = [clitic_pronoun.group(i_group) for i_group in range(1, 11) if
                               not clitic_pronoun.group(i_group) == None][0]
                    old_lemma = pos_tag
                    if TRACE:
...
def transformAdditionalLefffDict(additional_dict_lefff):
    ...
    for dict_key, dict_value in additional_dict_lefff.items():
        ...
        if TRACE and (line_number % 10000 == 0):
            print(dict_key, extractInflectedFromKey(dict_key, additional_dict_lefff),
                  extractPOSFromKey(dict_key, additional_dict_lefff), dict_value)
